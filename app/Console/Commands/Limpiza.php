    ...
    public function handle()
    {
        ...
        if ($servidoresExpirados->isEmpty()) {
            ...
            foreach ($servidoresExpirados as $servidor) {
                $rutaOriginal = $servidor->ruta;
                $this->line("Procesando Servidor expirado con el ID: {$servidor->id} (Estado actual: {$servidor->estado}, Ruta: {$rutaOriginal}, Expira: {$servidor->fecha_expiracion})");

                try {
                    if (!empty($rutaOriginal)) {
                        if (Storage::disk('public')->exists($rutaOriginal)) {
                            Storage::disk('public')->delete($rutaOriginal);
                            $this->info("-> Carpeta '{$rutaOriginal}' eliminado del disco 'public'.");
                        } else {
                            $this->warn("-> Carpeta '{$rutaOriginal}' no encontrado en el disco 'public', pero se procederá a marcar como expirado.");
                        }
                    } else {
        ...
        if ($servidoresAtascados->isEmpty()) {
            ...
            $this->info("Se encontraron {$servidoresAtascados->count()} servidor(es) atascado(s) para procesar.");

            foreach ($servidoresAtascados as $servidor) {
                $rutaOriginal = $servidor->ruta;
                $this->line("Procesando Servidor atascado con el ID: {$servidor->id} y ruta: {$rutaOriginal}");

                try {
                    if (!empty($rutaOriginal)) {
                        if (Storage::disk('public')->exists('mundos_pendientes/' . $rutaOriginal)) {
                            Storage::disk('public')->delete('mundos_pendientes/' . $rutaOriginal);
                            $this->info("-> Carpeta '{$rutaOriginal}' eliminado del disco 'public/mundos_pendientes'.");
                        } else {
                            $this->warn("-> Carpeta '{$rutaOriginal}' no encontrado en el disco 'public/mundos_pendientes', pero se procederá a marcar como error_procesamiento_timeout.");
                        }
                    } else {
                        $this->info("-> El servidor no tiene una ruta de carpeta especificada.");
                    }
                    ...
                    $this->error("-> Error al procesar servidor con el ID: {$servidor->id}. Mensaje: " . $e->getMessage());
                    Log::error("Error limpiando servidor atascado ID {$servidor->id}: " . $e->getMessage(), [
                        'servidor_id' => $servidor->id,
                        'ruta_archivo' => $rutaOriginal,
                        'exception' => $e
                    ]);
